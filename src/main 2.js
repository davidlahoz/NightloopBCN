...
async function main() {
  ...
  const refreshRenderLists = () => {
    ...
    for (const m of scene.meshes) {
      ...
      if (m === env.sky) { list.push(m); continue; }
      if (m.name.startsWith('car') || m.name.startsWith('wheel')) { list.push(m); continue; }
      // city module meshes opt in via metadata.mirror !== false
      if (m.metadata && m.metadata.nlMirror) list.push(m);
    }
